...
function header(doc, invoice, owner, client) {
...
export async function makeInvoicePdf(invoice, owner, client, lines) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'A4', margin: 0, info: { Title: `Facture ${invoice.number}`, Author: owner.name || 'Facturato' } });
    const chunks = [];
    doc.on('data', chunk => chunks.push(chunk));
    doc.on('end', () => resolve(Buffer.concat(chunks)));
    doc.on('error', reject);
    header(doc, invoice, owner, client);
    let y = 344;
    tableHead(doc, y);
    y += 40;
    for (const line of lines) {
      ...
      text(doc, short(line.description, 34), 37, y, { size: 9.5, width: 154 });
